...
# Clean 'epoch (ms)' column by converting to numeric and coercing errors
acc_df['epoch (ms)'] = pd.to_numeric(acc_df['epoch (ms)'], errors='coerce')
gyr_df['epoch (ms)'] = pd.to_numeric(gyr_df['epoch (ms)'], errors='coerce')

# Drop rows with NaN values in 'epoch (ms)' column
acc_df.dropna(subset=['epoch (ms)'], inplace=True)
gyr_df.dropna(subset=['epoch (ms)'], inplace=True)
