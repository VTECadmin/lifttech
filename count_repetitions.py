...
triceps_df = df[df["label"] == "triceps"]
biceps_df = df[df["label"] == "biceps"]

# --------------------------------------------------------------
# Function to get first set safely
# --------------------------------------------------------------
def get_first_set(df):
    if df.empty or df["set"].unique().size == 0:
        return pd.DataFrame()
    return df[df["set"] == df["set"].unique()[0]]

# --------------------------------------------------------------
# Get first sets
# --------------------------------------------------------------
bench_set = get_first_set(bench_df)
squat_set = get_first_set(squat_df)
row_set = get_first_set(row_df)
ohp_set = get_first_set(ohp_df)
dead_set = get_first_set(dead_df)
triceps_set = get_first_set(triceps_df)
biceps_set = get_first_set(biceps_df)

# --------------------------------------------------------------
# Visualize data to identify patterns
# --------------------------------------------------------------
plot_df = bench_df
if not plot_df.empty and plot_df["set"].unique().size > 0:
...
def count_reps(dataset, cutoff=0.4, order=10, column="acc_r"):
    ...
    exercise = dataset["label"].iloc[0].title() if not dataset.empty else "Unknown"
    category = dataset["category"].iloc[0].title() if not dataset.empty else "Unknown"
...
count_reps(triceps_set, cutoff=0.5)
count_reps(biceps_set, cutoff=0.5)
...
for s in df["set"].unique():
    ...
    if subset["label"].iloc[0] == "ohp":
        cutoff = 0.35
    if subset["label"].iloc[0] == "triceps":
        cutoff = 0.5
    if subset["label"].iloc[0] == "biceps":
        cutoff = 0.5
